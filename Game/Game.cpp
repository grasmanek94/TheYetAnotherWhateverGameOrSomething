...
void Game::Initialize(HWND window)
{
    ...
    */
	...
	if (m_client->GetInitCode()
		|| !m_client->Create()
		|| !m_client->Good()
		|| !m_client->Connect("127.0.0.1", 0xBEEF))
	{
		throw  "An error occurred while trying to create an ENet object.";
	}
}
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	if (m_client->Pull())
	{
		ENetEvent event = m_client->Event();
		switch (event.type)
		{
		case ENET_EVENT_TYPE_CONNECT:
			swprintf_s(debugText, L"connected to %x:%u.\n",
...
void Game::Render()
{
	...
	Vector2 origin = m_font->MeasureString(debugText) / 2.f;

	m_font->DrawString(m_spriteBatch.get(), debugText,
